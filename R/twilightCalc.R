i.preSelection <- function(datetime, light, LightThreshold){
  ...
  dt <- factor(format(floor_date(datetime, unit = "hour"), "%Y-%m-%d %H:%M:%S"))
                         ...
                         allTwilights=FALSE)
{
  ...
  raw <- data.frame(datetime=c(as.POSIXct(datetime,"UTC"),as.POSIXct(out$datetime,"UTC")),
                    light=c(light,rep(LightThreshold,nrow(out))),
                    type=c(rep(0,length(datetime)),out$type))
  ...
  opt <- data.frame(tFirst=as.POSIXct("1900-01-01 01:01","UTC"),
                    tSecond=as.POSIXct("1900-01-01 01:01","UTC"),
                    type=0)
  row <- 1
