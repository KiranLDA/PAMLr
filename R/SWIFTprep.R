...
#' @param ... any input for `sustainedACT()` function
#'
...
swiftPREP <- function(dta,
  ...
  # find when a bird is active
  classification <- sustainedACT(activity,
                                 method = method,
                                 duration_threshold = duration_threshold,
                                 keep_one_off_missclassifications = keep_one_off_missclassifications,
                                 toPLOT = toPLOT,
                                 ...)
  ...
  flight_list = classification$timetable
  names(flight_list) = c("start","end","duration")
  flight_list$date <- as.Date(flight_list$start)


  start = which(activity$date %in% flight_list$start)
  end = which(activity$date %in% flight_list$end)
  ...
  flight_list$pressure_change <- unlist(lapply(1:length(flight_list$total_daily_flight_number),
                                               function(x) {
                                                 sum(abs(diff(pressure$obs[min(which(pressure$date >= activity$date[start[nrow(flight_list[1:x,])]])):
                                                                             max(which(pressure$date <= activity$date[end[nrow(flight_list[1:x,])]]))
                                                                           ])))})
  )
  ...
  # )
  flight_list$P_dep_arr <- unlist(lapply(1:length(flight_list$total_daily_flight_number),
                                         function(x) abs(pressure$obs[min(which(pressure$date >= activity$date[start[nrow(flight_list[1:x,])]]))]-
                                                           pressure$obs[max(which(pressure$date <= activity$date[end[nrow(flight_list[1:x,])]]))] )
  )
  )

  # pressure$obs[min(which(pressure$date >= activity$date[start[nrow(flight_list[1:x,])]])):
  #                max(which(pressure$date <= activity$date[end[nrow(flight_list[1:x,])]]))
  #              ]
  ...
  # ))
  flight_list$pressure_range <- unlist(lapply(1:length(flight_list$total_daily_flight_number),
                                              function(x) max( pressure$obs[min(which(pressure$date >= activity$date[start[nrow(flight_list[1:x,])]])):
                                                                              max(which(pressure$date <= activity$date[end[nrow(flight_list[1:x,])]]))] )-
                                                min(pressure$obs[min(which(pressure$date >= activity$date[start[nrow(flight_list[1:x,])]])):
                                                                   max(which(pressure$date <= activity$date[end[nrow(flight_list[1:x,])]]))])
  ))
  ...
  if ("acceleration" %in% availavariable){
    ...
    #                                           function(x) sum(activity$act[which(activity$date == pressure$date[start[nrow(flight_list[1:x,])]]
    #                                                                              ):which(activity$date == pressure$date[end[nrow(flight_list[1:x,])]] ) ])))
    flight_list$sum_activity <- unlist(lapply(1:length(flight_list$total_daily_flight_number),
                                              function(x) sum(activity$act[start[nrow(flight_list[1:x,])]:end[nrow(flight_list[1:x,])]])
                                              )
                                       )
    ...
    flight_list$prop_resting <- unlist(lapply(1:length(flight_list$total_daily_flight_number),
                                              function(x) length(which(activity$act[start[nrow(flight_list[1:x,])]:end[nrow(flight_list[1:x,])]] == 0 )) /
                                                length(activity$act[start[nrow(flight_list[1:x,])]:end[nrow(flight_list[1:x,])]])
